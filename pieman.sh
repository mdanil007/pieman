...
def_var CREATE_ONLY_ARTIFACT false
...
if ${CREATE_ONLY_ARTIFACT}; then
    BUILD_TYPE="${IMAGE_ARTIFACT}"
fi
...
root_partition_size="$(python3 -c "import math; print(math.ceil($(calc_size "${R}") / 1024 / 1024))")"
...
"${IMAGE_FOR_RPI_WITH_MENDER_CLIENT}")
    image_size="$(create_image 4 fat32:100 "${total}" "${total}" "${MENDER_DATA_SIZE}")"
    info "${IMAGE} of size ${image_size}M was successfully created"
